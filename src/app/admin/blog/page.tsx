  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/blog/new" className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">
          Nytt innlegg
        </Link>
      </div>
      ...
      <ul className="space-y-4">
        {posts.map((post) => (
          <li key={post.id} className="flex items-center justify-between p-4 border rounded-md">
            <span className="font-semibold">{post.title}</span>
            <div className="space-x-2">
              <Link href={`/admin/blog/edit/${post.slug}`} className="text-blue-500 hover:underline">
                Rediger
              </Link>
              {/* TODO: Koble denne knappen til `deletePost` server action */}
              <button className="text-red-500 hover:underline">Slett</button>
            </div>
          </li>
        ))}
      </ul>
